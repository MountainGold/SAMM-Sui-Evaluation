...
use sui_json_rpc_types::{Coin, SuiObjectDataOptions, SuiTypeTag, ObjectChange, SuiExecutionStatus, SuiTransactionBlockResponse};
...
pub async fn split_coins_test(client: SuiClient, sender: SuiAddress, coin_id:ObjectID, gas_object: ObjectID, split_count: u64)
    -> Result<Vec<ObjectID>, anyhow::Error>
{

    let total_process = ceil_divide(split_count, MAX_COIN_PER_PROCESS as u64)?;
    let mut this_split_count = total_process;
    let mut result: Vec<ObjectID> = vec![];

    if 1 > 2
    // if total_process > MAX_COIN_PER_PROCESS as u64
    {
        let gas_obj_list = get_gas_obj_one_layer(MAX_COIN_PER_PROCESS, sender).await?;
        let num_sub_process = ceil_divide(total_process, MAX_COIN_PER_PROCESS as u64)? as usize;

    }
    else 
    {
        //let mut tasks = Vec::new();
        let gas_obj_list = get_gas_obj_one_layer(total_process as usize, sender).await?;
        let Task_queue = split_coins(client.clone(), sender, coin_id, gas_object, total_process).await?;
        let sub_split_count = ceil_divide(split_count, total_process)?;
        for i in 0..total_process
        {
            let client_clone = client.clone();
            let this_gas_obj = gas_obj_list.clone()[0].parse::<ObjectID>()?.clone();
            let this_coin_id = *(Task_queue.get(i as usize).unwrap());
            // println!("{:?}", this_coin_id);
            // let task = tokio::spawn(async move {
            // split_coins(client_clone, sender, this_coin_id, this_gas_obj, sub_split_count).await
            // });
            result.extend(split_coins(client_clone, sender, this_coin_id, this_gas_obj, sub_split_count).await?);
        }
        // for task in tasks 
        // {
        //     let result_part = task.await??;
...
pub async fn omniswap_builder(client: SuiClient, sender: SuiAddress, gas_object: ObjectID)-> Result<ContractInfo, anyhow::Error>
{
    // Initiate move package
    let source_file_path = "../omniswap/Move_backup.toml";
    let target_file_path = "../omniswap/Move.toml";
    ...
    let test_transaction_sender = TestTransactionSender::new(sender, gas_object, client.clone());
    let transaction_response = test_transaction_sender.publish_package("omniswap").await?;
    ...
    let test_transaction_sender = TestTransactionSender::new(sender, gas_object, client.clone());
    let transaction_response = test_transaction_sender.publish_package("omniswap/test_coins").await?;
    // println!("{:?}", transaction_response.clone());
...
{
    let destination_file = "../omniswap/Move.toml"; // 替换为目标文件的路径
...
pub async fn omniswap_testcoin_builder(client: SuiClient, sender: SuiAddress, 
